...
if (! function_exists('mfn_breadcrumbs')) {
	function mfn_breadcrumbs($params = false)
	{
        ...
        // single custom post archives | ACF fix
			...
			while ($parent_id) {
				$page = get_page($parent_id);
				$parents[] = '<a href="'. esc_url(get_permalink($page->ID)) .'">'. htmlentities( get_the_title($page->ID) ) .'</a>';
				$parent_id = $page->post_parent;
			}
			...
			$breadcrumbs[] = '<a href="'. esc_url(get_permalink($post->ID)) .'">'. htmlentities( get_the_title(mfn_ID()) ) .'</a>';
