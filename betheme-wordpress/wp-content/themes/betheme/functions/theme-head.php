...
function mfn_styles_templates() {
 	...
 	if( 'inline' == mfn_opts_get('local-styles-location') && empty( $_GET['visual'] ) ) {
 		...
 		if( is_singular() ) {
 			...
 			}else if( is_page() && !empty($mfn_global['archive_template']) && (is_page_template('template-portfolio.php') || ( function_exists('is_woocommerce') && is_shop() ) ) ){
...
if (! function_exists('mfn_body_classes')) {
	function mfn_body_classes($classes)
	{
		...
		if( function_exists('is_woocommerce') && is_product() && !isset($shop_icons_hide['wishlist']) && isset($wishlist_position[0]) ) {
