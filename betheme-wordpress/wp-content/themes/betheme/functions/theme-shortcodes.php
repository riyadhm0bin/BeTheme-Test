...
if (! function_exists('sc_shop_products')) {
	function sc_shop_products($attr, $sample = false){
		...
		$output  .= '<ul class="products '. esc_attr(implode(' ', $classes)) .' '.(!empty($attr['product_badge_onsale_position']) ? 'mfn-product-badges-right' : 'mfn-product-badges-left').'">';
			while ( have_posts() ) {
				the_post();
				global $product;
...
if (! function_exists('sc_clients')) {
	function sc_clients($attr, $content = null)
	{
		...
		$output  = '<ul class="clients clients_ul clearfix '. esc_attr($class) .'">';
			...
			if ($clients_query->have_posts()) {
				...
				while ($clients_query->have_posts()) {
					...
					$output .= '<li class="clients_li" style="width:'. esc_attr($width) .'%">';
